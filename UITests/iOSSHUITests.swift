...
final class iOSSHUITests: XCTestCase {
    @MainActor func testJapaneseKanaKeyboardComposesAndConfirmsLocally() async throws {
        ...
        app.launch()
        try app.buttons["addFirstHost"].tapWhenReady()
        try app.textFields["hostName"].typeWhenReady("Japanese input")
        try app.textFields["hostAddress"].typeWhenReady("192.0.2.1")
        try app.textFields["hostUsername"].typeWhenReady("tester")
        try app.buttons["saveHost"].tapWhenReady()
        try app.buttons["host-Japanese input"].tapWhenReady()
        XCTAssertTrue(app.secureTextFields["Password"].waitForExistence(timeout: 10))
        try app.buttons["Cancel"].tapWhenReady()
    ...
    @MainActor func testTerminalViewportSurvivesKeyboardForegroundAndReconnect() async throws {
        ...
        app.launch()
        try app.buttons["addFirstHost"].tapWhenReady()
        try app.textFields["hostName"].typeWhenReady("Viewport")
        try app.textFields["hostAddress"].typeWhenReady("192.0.2.1")
        try app.textFields["hostUsername"].typeWhenReady("tester")
        try app.buttons["saveHost"].tapWhenReady()
    ...
    @MainActor func testTerminalConnectionCanBeCanceled() throws {
        ...
        app.launch()
        try app.buttons["addFirstHost"].tapWhenReady()
        try app.textFields["hostName"].typeWhenReady("Terminal test")
        try app.textFields["hostAddress"].typeWhenReady("192.0.2.1")
        try app.textFields["hostUsername"].typeWhenReady("tester")
        try app.buttons["saveHost"].tapWhenReady()
    ...
    @MainActor func testHostCreationEditingAndDeletion() throws {
        ...
        try app.textFields["hostName"].typeWhenReady("Development")
        try app.textFields["hostAddress"].typeWhenReady("192.0.2.1")
        try app.textFields["hostUsername"].typeWhenReady("developer")
        try app.buttons["saveHost"].tapWhenReady()
    ...
    @MainActor func testSettingsAndHostValidation() throws {
        ...
        try app.buttons["addFirstHost"].tapWhenReady()
        try app.textFields["hostName"].typeWhenReady("Bad host")
        try app.textFields["hostAddress"].typeWhenReady("bad host")
        try app.textFields["hostUsername"].typeWhenReady("root")
        XCTAssertFalse(app.buttons["saveHost"].isEnabled)
    }
}
