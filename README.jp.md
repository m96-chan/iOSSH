...
現在の iPad ビルドは iPhone と共通の単一セッション画面を使う。[iPad UI](#ipad-ui未実装の仕様) に記載する専用ワークスペースと接続タブは、次に取り組む設計であり、まだ実装していない。
...
- 描画リソースは表示中のターミナル分だけ保持し、非表示の GPU キャッシュを解放する。初期目標は **アプリ全体でデコード済み Kitty 画像 64 MiB**、既存の **1 画像 16 MiB** の上限、および上限付きの表示中グリフアトラスとする。各セッションの scrollback も上限を維持する。メモリ警告では画像・描画キャッシュを破棄できるが、SSH を自動切断したり端末のテキストを消したりしない。これは実装目標であり、現状の画像上限はセッション単位。

#### 実装順序

1. `ConnectionModel` の所有を `TerminalScreen` からワークスペースのセッションストアへ移し、選択中のモデルを端末画面へ渡す。既存の iPhone の接続フローを確認する。
2. 1 セッションで iPad の適応型サイドバー・詳細領域・空画面を実装し、ウィンドウとキーボードのリサイズに対応する。
3. 接続を保持するタブとコンパクトな選択メニューを追加し、セッションごとの入力先、認証表示、ワークスペースのショートカットを実装する。
4. 非表示セッションの処理調整とアプリ全体のリソース上限を追加し、以下の iPad 受け入れ確認を終えてから実装済みとする。

---

## 3. アーキテクチャ

図は 1 セッションの構成を表す。計画中の iPad ワークスペースでは、セッションストアが順序付きの `ConnectionModel` 群と選択中のセッション ID を持つ。各モデルが自身の `SSHSession` と `TerminalEngine` を保持し、選択中のモデルだけを表示中の `TerminalView` へ接続する。iPhone も同じ所有関係で 1 セッションを扱う。ビューが消えただけで接続を閉じてはならない。
...
- **次: iPad ワークスペース** — セッションの所有、適応型サイドバー・詳細領域、接続を保持するタブ、キーボード・ポインタ対応、リソースと受け入れ確認
