...
| --- | --- | --- |
| iPhone | ホスト一覧から現在の単一セッション画面へ進む。ナビゲーションバーは置かず、閉じる操作とセッションメニューをターミナル上にフローティング表示する | 画面を使う 1 つのターミナル。常設タブ列は置かない |
...
iPhone の単一セッション画面はナビゲーションバーを持たず、ターミナルの色を画面の端まで広げる。ターミナルの高さを削る常設の行を作らないための配置とする。出力は各行の左端から始まるため、閉じる操作とセッションメニューは右上の角に丸いフローティングボタンとしてまとめ、接続先名と接続状態は反対側のバッジに常時表示する。この画面はモーダルとし、抜ける手段は閉じるボタンだけとする。戻るジェスチャを設けず、誤操作で接続中のセッションを落とさない。

詳細領域の最上部 1 行に、サイドバーの表示切り替え、接続タブ、セッション操作をまとめる。接続先を中央に表示する別のタイトル行は置かず、ソフトウェアキーボード表示中のターミナルの高さを確保する。設定はサイドバー下部に置き、狭い表示では **Hosts** 選択画面から開く。ターミナルは角丸とし、小さな内側余白で文字が縁にかからないようにする。下端はサイドバーの Settings 行と同じ高さ、すなわち安全領域の下端に合わせる。サイドバーの内容と同じところまでグリッドを伸ばしつつ、ホームインジケータには重ねない。
