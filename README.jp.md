...
日本語と Starship / Nerd Font の記号に対応する HackGen Console NF を標準同梱。未収録の日本語文字は、追加したフォントを使う場合も含め、同梱の Noto Sans CJK JP で補う。設定から「ファイル」の等幅 TTF / OTF フォントを追加し、iOSSH 内で選択できる。グリフを端末のセル幅に収め、アプリ復帰・再接続時もキーボードと補助キー行に合わせて表示領域を更新する。日本語入力には UIKit の変換機能を使い、確定した文字だけを送信する。
