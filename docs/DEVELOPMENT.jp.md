...
標準フォントとして **HackGen Console NF** を使用し、初期サイズは **iPhone が 8.5pt、iPad が 9pt**。日本語と Starship 用の Nerd Font 記号に対応する。シェルの出力は 80 桁を前提とし、縦向きの iPhone がこのアプリで最も狭いグリッドになる。8.5pt なら 2 倍密度の 375pt 幅でも 3 倍密度の 390pt 幅でも 83 桁入るが、9pt では 78 桁しか入らない。セル幅は端末のピクセルへ切り上げるため、設定のフォントサイズは 0.5pt 刻みとする。1pt 刻みでは 80 桁を保てるサイズを飛び越える。iPad は 9pt でも十分な幅がある。保存済みのフォントサイズは維持する。行高はフォント本来の高さを端末のピクセル単位に丸め、小さいサイズでブロックアートを縦長にする余分な行間は追加しない。未収録の文字は、追加フォントの使用時も含め、同梱 **Noto Sans CJK JP** の Regular / Bold で補う。絵文字は OS のカラー絵文字フォールバックを利用する。半角と全角の送り幅は 1:2。幅の広い記号もパーサが指定するセル数に収め、Powerline の区切りはセルの端まで描画する。設定では選択中のフォントのプレビュー、日本語フォールバック、すべての同梱フォントのライセンスを確認できる。サーバー側のプロンプトの文字幅設定も、端末の Unicode 幅と一致させる必要がある。

### パース性能

出力経路で最も遅いのは VT パースである。`scripts/vt_throughput_bench.c` は全画面の truecolor 再描画（105×95 セル、各セルが上半分ブロック＋前景色＋背景色、約 384KB）を libghostty-vt に流し、同じバイト列を Release ビルドの TerminalCore テストで `SwiftTermEngine` に流して比較する。Apple silicon の Mac での実測:

| エンジン | 1フレーム | スループット |
| --- | --- | --- |
| SwiftTerm（現行） | 15.5 ms | 24 MB/s |
| libghostty-vt | 1.69 ms | 228 MB/s |

計測用のハーネスはライブラリのヘッダだけに依存する単体の C であり、アプリ・エンジンのラッパー・スナップショットを挟まずにパーサ単体を測る。リンクするライブラリは `scripts/build_ghostty_vt.sh` で生成する。実行手順はファイル冒頭のコメントに記載。
...
PTY へ送るウィンドウサイズには、実測したセルサイズから求めたピクセルサイズも必ず含める。画像を描画するツールはエスケープシーケンスではなくリモート tty のウィンドウサイズからピクセル数を読むため、0 のままだと描画を拒否する。フォントサイズの変更では行数・列数は変わらずピクセルサイズだけが変わるので、その場合も単独でウィンドウ変更を送る。`CSI 14 t` と `CSI 16 t` も同じ実測値で応答する。
...
[ストア掲載文と審査メモ](APP_STORE.jp.md)で、App Store Connectに必要な残りの入力を管理する。公開用の[サポートページ](SUPPORT.jp.md)と[プライバシーポリシー](PRIVACY.jp.md)は、アプリ内の **About** からリンクする。アプリのプライバシーマニフェストは、アプリ内設定用のUserDefaultsへのアクセスを理由 `CA92.1` で申告している。
